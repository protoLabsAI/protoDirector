...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    "prompt": ["type": "string", "description": "Required for text-driven models. TTS uses it as spoken text. Music uses it as the description — for ACE-Step write a CAPTION of comma-separated tags (genre first, then instruments, mood, an explicit BPM, vocal style), not prose; 3–7 tags, no contradictory genres. Omit for Voice Cleanup and Dubbing."],
                    "name": ["type": "string", "description": "Display name for the asset in the media library. Defaults to first 30 chars of prompt."],
                    ...
                    "lyrics": ["type": "string", "description": "Music with vocals. Section tags — ACE-Step: [intro]/[verse]/[chorus]/[bridge]/[instrumental]/[outro] at ~2–3 words per second, short lines; MiniMax: [Verse]/[Chorus]. For a pure instrumental set instrumental=true and put only [instrumental] here. If omitted and instrumental=false, the model writes lyrics from the prompt."],
                    ...
                    "instrumental": ["type": "boolean", "description": "Music models only. true = no vocals when the selected model supports it (ACE-Step: also put only [instrumental] in lyrics). Defaults to false."],
                    "duration": ["type": "integer", "description": "Length in seconds. ACE-Step music: the track length. ElevenLabs Music: 3–600. Sonilo text-to-music: up to 600. Video-to-audio models default to the source span; pass a value to request a different output length. Voice Cleanup and Dubbing always preserve the source length. Ignored by TTS, MiniMax, and Lyria 3 Pro."],
                    "seed": ["type": "integer", "description": "Optional fixed seed (ACE-Step music). Reuse a seed that produced a good take to keep results consistent while you vary the caption; omit for a fresh random result."],
                    "quality": ["type": "string", "enum": ["standard", "high"], "description": "ACE-Step music fidelity tier. 'standard' (default) = fast turbo, best for iterating. 'high' = sft tier (~2× slower) for a final take. Ignored by other models."],
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "prompt": ["type": "string", "description": "Caption for the new audio — comma-separated tags (genre, instruments, mood, explicit BPM, vocal style), not prose. Required for edit; optional for extend/repaint."],
                    "lyrics": ["type": "string", "description": "Optional. Lyrics with section tags [intro]/[verse]/[chorus]/[bridge]/[instrumental]/[outro] — mainly for 'edit'."],
